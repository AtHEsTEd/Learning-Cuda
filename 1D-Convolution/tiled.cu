...
__global__ void convolution_1d(int* array, int* result, int n, int padded)
{
	...
	// Size of padded smem array
	int n_padded = blockDim.x + padded;

	// Offset for second load
	int offset = threadIdx.x + blockDim.x;

	// Global offset in DRAM
	int g_offset = blockDim.x * blockIdx.x + offset;

	// Load lower elements starting at halo to ensure only one divergergence
	smem_array[threadIdx.x] = array[tid];

	// Load remaining upper elements
	if (offset < n_padded) smem_array[offset] = array[g_offset];
	__syncthreads();

	int temp = 0;

	for (int i = 0; i < MASK_LENGTH; i++) temp += smem_array[threadIdx.x + i] * mask[i];
...
int main()
{
	...
	convolution_1d << < GRID, THREADS, SMEM >> > (device_array, device_result, n, 2 * r);
